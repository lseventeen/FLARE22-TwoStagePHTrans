...
from data.utils import load_data, clip_and_normalize_mean_std, resize_segmentation, change_axes_of_image, crop_image_according_to_mask, create_two_class_mask
...
class data_process(object):
    def __init__(self, config, is_overwrite=True):
        ...
        self.image_path = config.DATASET.TRAIN_IMAGE_PATH
        self.mask_path = config.DATASET.TRAIN_MASK_PATH
        self.preprocess_coarse_path = config.DATASET.COARSE.PROPRECESS_PATH
        self.preprocess_fine_path = config.DATASET.FINE.PROPRECESS_PATH
    ...
    def process(self, image_id):
        ...
        crop_image_resize = resize(crop_image, self.fine_size,
                                   order=3, mode='edge', anti_aliasing=False)
